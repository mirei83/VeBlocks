...
apt-get update
apt-get -y install build-essential libgflags-dev libsnappy-dev zlib1g-dev libbz2-dev liblz4-dev git mysql-server net-tools curl bc jq
...
### Clone VeBlocks
cd ~
git clone https://github.com/mirei83/VeBlocks.git
cd VeBlocks
chmod +x VeBlocks_import.sh
### Import MySQL Datavase Structure
mysql < VeBlocks.sql
cd ~
